...
import 'package:http/http.dart'
    as http; // 👈 Kept this one, removed the duplicate above it
...
class DashboardScreen extends StatefulWidget {
  const DashboardScreen({super.key, this.enableIntegrations = true});
...
// This function handles initializing and listening to your voice
  void _listenToVoice() async {
    if (!_isListening) {
      bool available = await _speech.initialize(
        onStatus: (val) => _log('Speech Status: $val'),
        onError: (val) => _log('Speech Error: $val'),
      );
      if (available) {
  ...
  void initState() {
    super.initState();
    if (widget.enableIntegrations) {
      _databaseRef = FirebaseDatabase.instance.ref();
      _dbRef = FirebaseDatabase.instance.ref("completed_cycles");
  ...
  void _log(String message, {Level level = Level.INFO}) {
    Logger('SmartSort').log(level, message);
  }
  ...
  void _saveWalletToDevice() async {
    ...
    await prefs.setString(
        'saved_wallet_address', _userWalletController.text.trim());
  ...
  Future<void> _loadSavedWallet() async {
    final SharedPreferences prefs = await SharedPreferences.getInstance();
    final String? savedWallet = prefs.getString('saved_wallet_address');
    if (!mounted || savedWallet == null || savedWallet.isEmpty) return;

    _userWalletController.text = savedWallet;
    setState(() {
  ...
  void initiateWasteDisposal() async {
    ...
    try {
      ...
      final dynamic balanceData =
          await _solanaClient.rpcClient.getBalance(_walletAddress);
      ...
      _log("Gas deduction fetch delayed: $e", level: Level.WARNING);
    }
  ...
  Future<void> _updateBalance() async {
    try {
      final dynamic balanceData =
          await _solanaClient.rpcClient.getBalance(_walletAddress);
      final int lamports =
          (balanceData is int) ? balanceData : (balanceData.value as int);
      setState(() {
  ...
  void _activateFirebaseStream() {
    _log("Initializing Firebase stream listener...");

    _databaseRef.child('SmartSort').onValue.listen((DatabaseEvent event) {
      _log("Firebase updated! Data found: ${event.snapshot.value}");

      final data = event.snapshot.value as Map<dynamic, dynamic>?;
      if (data != null) {
        setState(() {
          int incomingWasteLevel =
              int.tryParse(data['waste_level']?.toString() ?? "0") ?? 0;
          ...
          if (incomingWasteLevel >= 90) {
            ...
            if (wasteLogs.isEmpty || !wasteLogs[0].contains("🚨 [ALERTA]")) {
              wasteLogs.insert(0,
                  "🚨 [ALERTA] CRITICAL: Bin full ($incomingWasteLevel%). Maintenance ticket pushed out.");
            }
          } else {
  ...
  Future<void> _recordSortingOnChain(String wasteType) async {
    ...
    if (isMaintenanceMode) {
      _log(
        "Workflow Step 6: Operation blocked. SmartSort is locked inside Maintenance Mode.",
        level: Level.WARNING,
      );
      setState(() {
        if (!wasteLogs[0].contains("🚫 SORTING BLOCKED")) {
          wasteLogs.insert(0,
              "🚫 SORTING BLOCKED: Deposit rejected due to capacity limits. Awaiting sweep.");
        }
      });
...
// 👇 PASTE THE PIECE OF CODE HERE (At the bottom of your successful try block) 👇
    if (wasteLevel >= 90) {
      await sendTelegramAlert("🚨 *CRITICAL NODE ALERT* 🚨\n\n"
    ...
    _log("Workflow Step 1: User deposited $wasteType");

    // 2. Workflow: Reward points are recorded in the backend (Calculate points/credits)
    double calculatedCredit =
        wasteType.toLowerCase().contains("wet") ? 0.05 : 0.10;

    setState(() {
      digitalRewardCredits += calculatedCredit;
    });
    _log(
      "Workflow Step 2: Points recorded. Total user credits: $digitalRewardCredits SOL",
    );
    ...
    if (destinationInput.isEmpty) {
      setState(() {
        wasteLogs.insert(0,
            "ℹ️ Account Balance: +$calculatedCredit Credits logged locally (No wallet set)");
      });
      return;
    }
    ...
    try {
      ...
      _log("Solana execution skipped or failed: $e", level: Level.WARNING);
  ...
  Future<void> sendTelegramAlert(String message) async {
    const String token = "8919742610:AAFsl2b1iXXsSF9YHv_ceI8QsFtIeziSnYw";
    const String chatId = "-5356770635";
    ...
    try {
      ...
      if (response.statusCode == 200) {
        _log("🚀 Telegram alert pushed successfully!");
      } else {
        _log(
          "❌ Telegram failed. Status code: ${response.statusCode}",
          level: Level.WARNING,
        );
      }
    } catch (e) {
      _log("⚠️ Error sending Telegram request: $e", level: Level.WARNING);
    }
  }
  ...
  Widget build(BuildContext context) {
    return DefaultTabController(
      ...
      child: Scaffold(
        ...
        appBar: AppBar(
          title: const Text('SmartSort Ecosystem',
                  ...
                  Padding(
                    ...
                    child: Row(
                      mainAxisAlignment: MainAxisAlignment
                          .start, // Pulls everything to the far left
                      crossAxisAlignment: CrossAxisAlignment
                          .center, // Centers them vertically with each other
                      children: [
                        // 1. App Logo on the left
                        Image.asset(
                          'assets/logo.png',
                          width:
                              205, // Slightly smaller so it blends cleanly next to text
                          height: 205,
                          fit: BoxFit.contain,
                        ),

                        const SizedBox(
                            width: 16), // Clear gap between the logo and text
                          ...
                          children: [
                            Text(
                              ...
                              style: TextStyle(
                                ...
                                fontWeight:
                                    FontWeight.w900, // Extra thick font weight
                                letterSpacing:
                                    0.5, // Tight, clean modern professional spacing
                                color: Colors
                                    .white, // Crisp white color pops cleanly off a dark theme
                              ),
                            ),
                            Text(
                              ...
                              style: TextStyle(
                                ...
                                color: Colors
                                    .tealAccent, // Neon accent color for premium contrast
                                letterSpacing: 1.5,
                              ),
                            ),
                          ],
                      ...
                      child: Column(
                        ...
                        children: [
                          Row(
                            ...
                            children: [
                              const Text('SmartSort Live Bin Status',
                                  style: TextStyle(
                                      color: Colors.grey,
                                      fontSize: 13,
                                      fontWeight: FontWeight.bold)),
                              if (isMaintenanceMode)
                                const Text('⚠️ MAINTENANCE ACTIVE',
                                    style: TextStyle(
                            ...
                            children: [
                              Column(
                                children: [
                                  ...
                                  const Text('System Level',
                                      style: TextStyle(
                                          color: Colors.grey, fontSize: 11)),
                                  Text(systemState.toUpperCase(),
                                      style: TextStyle(
                                          color: isMaintenanceMode
                                              ? Colors.redAccent
                                              : Colors.white,
                      ...
                      children: [
                        ...
                        IconButton(
                          ...
                          onPressed: () {
                            setState(() {
                              _isWalletSaved =
                                  false; // Opens up the input field again
                            });
                          },
                        ...
                        Expanded(
                          child: TextField(
                            ...
                            decoration: const InputDecoration(
                                labelText: "Enter Solana Wallet Address"),
                          ),
                        ),
                        ElevatedButton(
                  ...
                  Card(
                    ...
                    child: Padding(
                      ...
                      child: Column(
                        crossAxisAlignment: CrossAxisAlignment.start,
                        children: [
                          const Text('Hardware Actuator & Motor Overrides',
                              style: TextStyle(
                          ...
                          const Text('Voice Command Interface',
                              style: TextStyle(
                                  color: Colors.grey,
                                  fontSize: 11,
                                  fontWeight: FontWeight.bold)),
                  ...
                  Card(
                    ...
                    child: Padding(
                      ...
                      child: Column(
                        crossAxisAlignment: CrossAxisAlignment.start,
                        children: [
                          const Text('Your Waste Disposal Cycle History',
                              style: TextStyle(
                                  color: Colors.grey,
                                  fontSize: 13,
                                  fontWeight: FontWeight.bold)),
                            ...
                            child: ListView.builder(
                              ...
                              itemBuilder: (context, index) {
                                String friendlyLog = wasteLogs[index]
                                    .replaceAll("🔬 [Demo Mode Transfer] ",
                                        "♻️ Cycle Logged: ")
                                    .replaceAll(
                                        "⛓️ Tx:", "🔗 Blockchain Receipt ID:")
                                    .replaceAll("ℹ️ Account Balance:",
                                        "🌱 Credits Saved:");
                      ...
                      child: Column(
                        ...
                        children: [
                          Row(
                            mainAxisAlignment: MainAxisAlignment.spaceBetween,
                            children: [
                              const Text('Admin Machine Node Wallet',
                                  style: TextStyle(
                                      color: Colors.grey,
                                      fontSize: 13,
                                      fontWeight: FontWeight.bold)),
                              IconButton(
                                ...
                                onPressed: () {
                                  ...
                                  ScaffoldMessenger.of(context).showSnackBar(
                                    const SnackBar(
                                        content:
                                            Text('Admin Node Address copied!')),
                                  );
                                },
                              ),
                            ],
                          ...
                          Row(
                            mainAxisAlignment: MainAxisAlignment.spaceBetween,
                            children: [
                              const Text('Gas Reserve Balance:',
                                  style: TextStyle(
                  ...
                  Card(
                    ...
                    child: Padding(
                      ...
                      child: Column(
                        crossAxisAlignment: CrossAxisAlignment.start,
                        children: [
                          const Text('Hardware Fleet Diagnostics',
                              style: TextStyle(
                          ...
                          Row(
                            ...
                            children: [
                              ...
                              Column(
                                children: [
                                  const Text('Volume Metrics',
                                      style: TextStyle(
                                          color: Colors.grey, fontSize: 11)),
                                  const SizedBox(height: 4),
                                  Text('$wasteLevel% Full',
                                      style: TextStyle(
                                          color: wasteLevel > 80
                                              ? Colors.redAccent
                                              : Colors.orangeAccent,
                              ...
                              Column(
                                children: [
                                  const Text('Last Sorted Object',
                                      style: TextStyle(
                                          color: Colors.grey, fontSize: 11)),
                                  const SizedBox(height: 4),
                                  Text(
                                    wasteLogs.isNotEmpty &&
                                            !wasteLogs[0].contains("Waiting")
                                        ? (wasteLogs[0]
                                                .toLowerCase()
                                                .contains("wet")
                                            ? "💧 WET WASTE"
                  ...
                  Card(
                    ...
                    child: Padding(
                      ...
                      child: Column(
                        crossAxisAlignment: CrossAxisAlignment.start,
                        children: [
                          const Text('Master System Activity & Audit Ledger',
                              style: TextStyle(
